...
Ninguna necesita el cartucho: se hacen sobre src/hyperrally.asm y
src/hyperrally.notes. Vigilan que el listado no se degrade sin que nadie se
...
ASM = os.path.join(RAIZ, "src", "hyperrally.asm")
NOTES = os.path.join(RAIZ, "src", "hyperrally.notes")
TRACE = os.path.join(RAIZ, "work", "hyperrally.trace.json")
...
class TestWeb(unittest.TestCase):
    ...
    def test_las_paginas_no_inventan_direcciones(self):
        ...
        with open(os.path.join(RAIZ, "hyperrally.rom"), "rb") as f:
            rom = f.read()
        t = Tracer(rom, ORG)
